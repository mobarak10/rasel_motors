<template>
    <div class="col-md-12">
        ...
        <table class="table table-borderless table-sm">
            ...
            <tbody>
            <tr
                v-for="(_saleDetail, saleDetailIndex) in sale.saleDetails"
                :key="saleDetailIndex"
            >
                ...
                <td>
                    ...
                    <span class="small">{{
                            _saleDetail.total_product_rest_quantities_in_unit[
                                "display"
                            ] || "No quantity for return"
                        }}</span>
                </td>
                <td>
                    <div class="input-group input-group-sm">
                        ...
                        <div class="input-group-append">
                                <span class="input-group-text">{{
                                    _saleDetail.product.product_unit_labels[
                                        _saleDetail.product.product_unit_labels
                                            .length - 1
                                    ]
                                }}</span>
                ...
                <td>
                    <ul class="list-group list-group-flush">
                        <li
                            ...
                            v-for="(_warehouse,
                                warehouseIndex) in _saleDetail.quantities"
                            :key="warehouseIndex"
                        >
                            <p class="m-0">{{ _warehouse.title }}</p>
                            <p class="m-0">
                                {{
                                _warehouse.sale_return_product_rest_quantity_warehouse_wise_in_unit
                                ?
                                _warehouse.sale_return_product_rest_quantity_warehouse_wise_in_unit.display
                                :
                                "No quantity for return"
                                }}
                            </p>
                            <div
                                ...
                                v-for="(_label, labelIndex) in _saleDetail
                                        .product.product_unit_labels"
                                :key="labelIndex"
                            >
                                <input
                                    type="number"
                                    ...
                                    min="0"
                                    :disabled="
                                            !_warehouse.sale_product_rest_quantity
                                        "
                                />
                                <div class="input-group-append">
                                        <span
                            ...
                            >
                                Error:
                                {{
                                errors[sale.id][_saleDetail.product.id][
                                _warehouse.id
                                ]
                                }}
                            </div>
                ...
                <td class="text-right">
                    {{
                    parseFloat(
                    lineTotals[_saleDetail.product_id] || 0
                    ).toFixed(2)
                    }}
                </td>
            </tr>
            <tr>
                ...
                <td
                    class="text-right border-bottom border-dark"
            ...
            <tr v-if="adjustToCustomerBalance">
                <td colspan="3"></td>
                <td class="text-right">
            ...
            <tr v-else>
                ...
                <td class="text-right">
                    ...
                    >
                        ...
                        <option
                            v-for="(_cash, cashIndex) in extras.cashes"
                            :value="_cash.id"
                            :key="cashIndex"
                        >{{ _cash.title }}</option
                        >
...
export default {
    ...
    computed: {
        ...
        customerCurrentBalance() {
            return parseFloat(this.total + parseFloat(this.sale.customer.balance)).toFixed(2);
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        addQuantity(event, productId, warehouseId, order) {
            // add to quantity
            ...
            if (!(warehouseId in this.returnQuantities.quantities[productId])) {
                this.$set(
                    this.returnQuantities.quantities[productId],
                    warehouseId,
                    {}
                );
            }
            this.$set(
                this.returnQuantities.quantities[productId][warehouseId],
                order,
                event.target.value
            );
        },
        calculateLineTotal() {
            ...
            this.$awn.asyncBlock(
                axios.post(baseURL + "user/calculate-sale-return-line-total", {
                    saleId: this.sale.id,
        ...
        returnProceed() {
            ...
            // if direct cash out from cash
            if (!this.adjustToCustomerBalance){
                //select cash
                if (!this.cashId) {
            ...
            this.$awn.asyncBlock(
                axios.post(baseURL + "user/pos/return", {
                    sale_id: this.sale.id,
                    quantities: this.returnQuantities.quantities,
                    adjustment: this.adjustment,
                    adjust_to_customer_balance: this.adjustToCustomerBalance,
