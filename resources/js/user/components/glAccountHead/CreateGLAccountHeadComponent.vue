...
<script>
	export default {
		// in app-user.js make a component named CreateGLAccountHeadComponent 
		...
		methods: {
			...
			saveGlHead(){
				this.$awn.asyncBlock(
					// in web route user/glAccountHead store route 
					axios.post(baseURL + 'user/glAccountHead', { // post means store method 
						name: this.name,
						type: this.type,
