        ...
        <div class="card-body">
            <form method="post" @submit.prevent="save" class="row">
                ...
                <div class="form-group col-md-12 required">
                    <label for="customers">Supplier</label>
                    <select id="customers" class="form-control" @change="getSupplierBalance" v-model="supplierId" required>
                        <option selected disabled :value="null">Choose one</option>
                        <option v-for="(customer, index) in suppliers" :value="customer.id" :key="index">{{ customer.name }}</option>
                    </select>
                </div>
                <!-- end get customer -->
