						...
						<tbody>
							@forelse($gl_accounts as $account)
								<tr>
									...
									<td>
										<a href="{{ route('admin.glAccount.status', $account->id) }}" title="Click here to {{ ($account->status) ? 'Inactive' : 'Active' }}">
											<button class="btn btn-{{ ($account->status) ? 'danger' : 'success' }}">{{ ($account->status) ? 'Active' : 'Inactive' }}</button>
										</a>
									</td>
									
									<td class="text-right">
										<a class="btn btn-primary" href="{{ route('admin.glAccount.show', $account->id) }}" title="Show GL account information.">
											<i class="fa fa-eye" aria-hidden="true"></i>
										</a>

										<a class="btn btn-primary" href="{{ route('admin.glAccount.edit', $account->id) }}" title="Change GL account information.">
											<i class="fa fa-pencil" aria-hidden="true"></i>
										</a>
											
										<a href="{{ route('admin.glAccount.index') }}" class="btn btn-danger" title="Trash" onClick="if(confirm('Are you sure, want to delete this record?')){event.preventDefault();document.getElementById('delete-form-{{ $account->id }}').submit();} else {event.preventDefault();}">
											<i class="fa fa-times" aria-hidden="true"></i>
										</a>

										<form action="{{ route('admin.glAccount.destroy', $account->id) }}" method="post" id="delete-form-{{ $account->id }}" style="display: none;">
											@csrf
											@method('DELETE')
										</form>
									</td>
                ...
                <div class="modal fade" id="newGLModal" tabindex="-1" role="dialog" aria-labelledby="insertModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form action="{{ route('admin.glAccount.store') }}" method="post">
                                @csrf
